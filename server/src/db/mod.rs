...
mod tests {
    ...
    async fn migrations_run_and_seed_statuses() {
        ...
        assert_eq!(
            applied, 29,
            "expected twenty-nine applied migrations (0001-0005, 0007-0029, 0100)"
        );
    ...
    /// `config_dir` (migration 0100) threads NewSession -> INSERT -> row, and a
    ...
    /// The production claim behind migration 0100: a row that already existed
    /// when the column was added keeps today's behaviour. Apply the chain up to
    /// 0029, insert a session through the pre-0100 column set, then apply the
    ...
    #[tokio::test]
    async fn a_row_created_before_0100_backfills_to_the_empty_config_dir() {
        use sqlx::migrate::Migrate;
        use sqlx::{sqlite::SqliteConnection, Connection};
        let dir = std::env::temp_dir().join(format!("supermux-mig0100-{}", uuid::Uuid::new_v4()));
        std::fs::create_dir_all(&dir).unwrap();
        ...
        for m in migrator.iter().filter(|m| m.version < 100) {
            conn.apply(m).await.expect("pre-0100 migration applies");
        }
        // The pre-0100 column set: `config_dir` does not exist yet to bind.
        sqlx::query(
        ...
        migrator
            .run(&mut conn)
            .await
            .expect("0100 applies over an existing row");
