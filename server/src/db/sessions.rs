...
pub struct Session {
    ...
    /// Which Claude login this session boots on (migration 0030): the directory
    /// exported as `CLAUDE_CONFIG_DIR` in the launch line. The empty string
    /// means "use the daemon default", which is every pre-0030 row and every
...
pub struct NewSession {
    ...
    /// The session's Claude config dir (migration 0030), or the empty string
