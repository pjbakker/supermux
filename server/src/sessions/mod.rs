...
pub struct SessionView {
    ...
    pub runtime: String,
    /// The Claude config dir this session boots on (migration 0030); the empty
...
pub struct CreateInput {
    ...
    /// `CLAUDE_CONFIG_DIR` in the launch line (migration 0030). Absolute path,
...
pub async fn create(state: &AppState, input: CreateInput) -> Result<SessionView, AppError> {
    ...
    // Per-session Claude config dir (migration 0030). Absent or blank keeps the
...
async fn resumable_handler(
    ...
    let s = db::sessions::get(&state.pool, &name)
        ...
        .ok_or_else(|| AppError::NotFound(format!("session '{name}'")))?;
    let dir = s.dir.clone();
    // Same rule as the launch line (migration 0030): the session's own config
