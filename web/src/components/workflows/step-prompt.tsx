// StepPrompt — a single textarea that doubles as a slash-command picker.
//
// Salvaged from `components/scheduler/prompt-field.tsx`, which was the best
// thing in the old UI and is untouched here apart from its name, its data
// source (`GET /api/workflows/commands`) and its types. Mirrors the Claude Code
// TUI input contract: the user types freely; a leading `/` opens an inline
// autocomplete over the user's REAL installed commands (skills + user/managed
// commands + claude.ai MCP connectors). Picking a row inserts the token and
// leaves the caret after it so the prompt keeps being typed.
//
// Why one field, not three: a step wants a bare prompt, a bare `/cmd`, or
// `/cmd then more text` — the same shape as the in-session terminal, so the
// muscle memory carries over.
//
// `splitCommandAndPrompt` / `mergeCommandAndPrompt` come with it, and they are
// the reason the two-line delivery survives: a slash line is delivered as its
// OWN submission, because a command concatenated with its prompt stops being
...
import type { WorkflowCommand } from '@/lib/api/workflows'
...
// workflows chunk for the sake of two functions (fase B3 T3.3/T3.4).
...
const SOURCE_ICON: Record<WorkflowCommand['source'], EntityRow['icon']> = {
  skill: Sparkles,
  command: Terminal,
  mcp: ServerCog,
}
...
const SOURCE_LABEL: Record<WorkflowCommand['source'], string> = {
  skill: 'Skill',
  command: 'Command',
  mcp: 'MCP connector',
}
...
interface StepPromptProps {
  ...
  /** REAL installed commands from `GET /api/workflows/commands`. */
...
export function StepPrompt({
  ...
  // this field — `/dcr` finds `/daily-code-review` — and it came across with it.
  ...
  const matches = React.useMemo(() => {
    if (slashQuery === null) return [] as WorkflowCommand[]
    return rankEntities(
      commands,
      slashQuery,
  ...
  //
  // WHAT DID NOT MOVE: every piece of this form's text plumbing.
        ...
        {open && (
          ...
          >
            <EntityPickerView
              ...
              testId="step-prompt-picker"
              rowTestId="step-prompt-row"
              emptyLabel={
                commands.length
