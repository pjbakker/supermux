 ...
 *
 * Adding per-agent ROWS (`SessionSummary.agents`) is exactly the change that
 * would tempt someone to wire "…and no agents are running" into that decision.
 * It reads as an improvement and it is the bug. So the three modules that own
 * the turn boundary and the wire→entry translation are pinned to ZERO mentions:
 *
 ...
 *
 * The rows are consumed ONLY by presentational components (`working-row`,
 * `activity-status`, `grok-roster`, the `live-layer` header). That list is the
 * invariant; this test is the part of it a machine can check.
 *
...
describe('the subagent display-only invariant', () => {
  for (const rel of TURN_BOUNDARY_FILES) {
    ...
    test(`${name} never reads the agents key`, () => {
      const src = read(rel)
      // `agent` alone is too broad (this app is full of agents); the wire key
      // and the row type are the two shapes that would actually smuggle the
      // signal into the turn decision.
      const hits = src
        .split('\n')
        .map((line, i) => [i + 1, line] as const)
        .filter(([, line]) => /\bAgentRow\b|\.agents\b|\bagents\s*[:?]/.test(line))
      expect(hits).toEqual([])
    })
  }
})
